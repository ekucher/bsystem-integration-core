...
func (a *app) listClients(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		writeJSON(w, http.StatusServiceUnavailable, map[string]string{"error": "EspoCRM adapter unavailable"})
		return
	}
	reader, ok := adapter.(crmReader)
	if !ok {
...
func (a *app) listContacts(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range contacts {
		...
		clientID := ""
		if item.AccountID != "" {
			mapped, err := a.db.CreateGlobalEntity(r.Context(), "client", "espocrm", item.AccountID, "", nil)
			if err == nil {
...
func (a *app) listIssues(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range issues {
		...
		if item.Project.ID > 0 {
			mapped, err := a.db.CreateGlobalEntity(r.Context(), "project", "redmine", strconv.Itoa(item.Project.ID), "", map[string]any{"name": item.Project.Name})
			if err == nil {
