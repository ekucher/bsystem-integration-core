...
func (a *app) listClients(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { writeJSON(w, http.StatusServiceUnavailable, map[string]string{"error": "RBAC store unavailable"}); return }
	...
	if !ok || !okReader { writeJSON(w, http.StatusServiceUnavailable, map[string]string{"error": "EspoCRM adapter not configured"}); return }
	accounts, err := reader.ListAccounts(r.Context(), requestLimit(r, 50))
	if err != nil { upstreamFailure(w, "espocrm", err); return }
	...
	for _, item := range accounts {
		...
		if err != nil { writeJSON(w, http.StatusServiceUnavailable, map[string]string{"error": "Global ID mapping failed"}); return }
...
func (a *app) listContacts(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range contacts {
		...
		clientID := ""
		if item.AccountID != "" {
			if mapped, mapErr := a.db.CreateGlobalEntity(r.Context(), "client", "espocrm", item.AccountID, "", nil); mapErr == nil { clientID = mapped.GlobalID }
		}
...
func (a *app) listProjects(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || !okReader { writeJSON(w, http.StatusServiceUnavailable, map[string]string{"error": "Redmine adapter not configured"}); return }
	projects, err := reader.ListProjects(r.Context(), requestLimit(r, 50))
	if err != nil { upstreamFailure(w, "redmine", err); return }
...
func (a *app) listIssues(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range issues {
		...
		if item.Project.ID > 0 {
			if mapped, mapErr := a.db.CreateGlobalEntity(r.Context(), "project", "redmine", strconv.Itoa(item.Project.ID), "", map[string]any{"name": item.Project.Name}); mapErr == nil { projectID = mapped.GlobalID }
		}
...
func (a *app) listDocuments(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || !okReader { writeJSON(w, http.StatusServiceUnavailable, map[string]string{"error": "Outline adapter not configured"}); return }
	documents, err := reader.ListDocuments(r.Context(), requestLimit(r, 50))
	if err != nil { upstreamFailure(w, "outline", err); return }
